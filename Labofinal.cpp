...
class RedBlack{
    private:
        Node *root;
    public:
        Node* createNode(int value, Color colorNode){
            Node *t = new Node;
            t->key = value;
        ...
        void insertAndFix(int value, Color colorNode){
            Node *v = createNode(value, colorNode);
            root = insert(root, v);
            fix(root, v);
        }
...
int main() {
    ...
    tree.insertAndFix(4, RED);
    tree.insertAndFix(3, RED);
    tree.insertAndFix(2, RED);
    tree.insertAndFix(1, RED);
